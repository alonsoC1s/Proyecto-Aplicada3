...
ggplot(data = alc) +
  geom_sf(aes(fill = ALCALDIA), show.legend=FALSE) +
  geom_sf_label(aes(label = ALCALDIA))
```
...
info |>
  ...
  ggplot(aes(fill=EST_TOTAL, geometry=geometry)) +
    geom_sf() + 
    labs(
      title = "Alcaldías coloreadas por total de estaciones de transporte público al 2020"
    )
```
