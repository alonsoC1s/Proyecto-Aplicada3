...
info_score <- dplyr::left_join(info, con_by_year, by=c("ALCALDIA", "AÑO")) |>
  ...
  dplyr::mutate(POBLACION = POBLACION / 1000) |>
  # Distancia al zócalo de metros a kilómetros
  dplyr::mutate(MEAN_DIST = MEAN_DIST / 1000) |> 
  # Moviendo el score de 0,1 a 1,2 por div entre 0
  dplyr::mutate(score = score + 1) |>
  dplyr::mutate(ALCALDIA = as.factor(ALCALDIA))
```
...
```{R}
lm <- lm(score ~ AÑO +  ALCALDIA + POBLACION + ZOC_DIST, data=info_score, na.action=na.omit)
summary(lm)
```
